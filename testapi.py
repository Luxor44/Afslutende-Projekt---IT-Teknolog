...
def get_protocolscan():
    ...
    # convert data to a list of dictionaries
        ...
        for row in rows:
                  id_value = row[0]
                  ipaddress_value = row[1]
                  macaddress_value = row[2]
                  ssltls_json = row[3]
                  heartbleed_json = row[4]
                  protocols_json = row[5]
        })
               # Deserialize JSON strings to dictionaries
                  ssltls_dict = json.loads(ssltls_json)
                  heartbleed_dict = json.loads(heartbleed_json)
                  protocols_dict = json.loads(protocols_json)
